...
class SentdeBot(sc2.BotAI):
    def __init__(self):
        self.ITERATIONS_PER_MINUTE = 180
        self.MAX_WORKERS = 71
    ...
    async def scout(self):
        if len(self.units(OBSERVER)) > 0:
            ...
            for rf in self.units(ROBOTICSFACILITY).ready:
    ...
    async def intel(self):
        ...
        if not HEADLESS:
            resized = cv2.resize(self.flipped, dsize=(1000, 720), fx=2, fy=2)
    ...
    async def build_offensive_force(self):
        for sg in self.units(STARGATE).ready:
        ...
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            if self.iteration > self.do_something_after:
                if choice == 0:
                    ...
                    if len(self.enemy_units) > 0:
                        target = self.enemy_units.closest_to(random.choice(self.units(NEXUS)))
